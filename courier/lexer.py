...
class Lexer(object):
    """
    Tokenizers an input stream containing Courier records and returns the tokens.
    """
        ...
        while self.has_more:
            ...
            if self.matches_symbol("["):
                ...
                while self.matches_func(lambda x: x == "_" or x.isalnum()):
                    nextch,_,_,_ = self.get_chars()
                    curr_tok_value += nextch
                if curr_tok_value == "import":
                    return make_token(TokenType.KW_IMPORT)
                elif curr_tok_value == "namespace":
                    return make_token(TokenType.KW_NAMESPACE)
                elif curr_tok_value == "as":
                    return make_token(TokenType.KW_AS)
                elif curr_tok_value == "with":
                    return make_token(TokenType.KW_WITH)
                else:
